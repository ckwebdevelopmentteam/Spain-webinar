...
const FAN_POSITIONS = [
  { rot: -21, scale: 0.76, x: -33, y: 7.6, zIndex: 1 },
  { rot: -14, scale: 0.84, x: -23, y: 4.2, zIndex: 2 },
  { rot: -7,  scale: 0.92, x: -12, y: 1.6, zIndex: 3 },
  { rot: 0,   scale: 1.16, x: 0,   y: 0.2, zIndex: 25 },
  { rot: 7,   scale: 0.92, x: 12,  y: 1.6, zIndex: 3 },
  { rot: 14,  scale: 0.84, x: 23,  y: 4.2, zIndex: 2 },
  { rot: 21,  scale: 0.76, x: 33,  y: 7.6, zIndex: 1 },
];
...
function getSlotConfig(totalCards: number, slot: number) {
  ...
  return {
    rot: distance * 21,
    scale: isCenter ? 1.16 : (0.95 - 0.22 * absDistance),
    x: distance * 33,
    y: isCenter ? 0.2 : (1.6 + absDistance * absDistance * 6.0),
    zIndex: isCenter ? 25 : (10 - Math.abs(slot - center)),
  };
}
...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  useEffect(() => {
    // Keep only the active center card video unmuted, restart from beginning and play
  ...
  // Fallback auto-cycle for cards without video
  useEffect(() => {
    const activeCard = cards[centerIndex];
    if (!activeCard?.videoUrl) {
      const timer = setTimeout(() => {
        cycle("right");
      }, 6000);
      return () => clearTimeout(timer);
    }
  }, [centerIndex, cards, cycle]);
  ...
  return (
    <section className="flex flex-col items-center w-full py-0 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            ...
            const cardClass = `fan-card ${isCenter ? "fan-card-active ring-2 ring-white/40 z-30" : "opacity-80 hover:opacity-100"}`;
            const image = (
              <div className="relative w-full h-full overflow-hidden bg-neutral-950 rounded-[inherit]">
                {card.videoUrl ? (
                  <div className="relative w-full h-full">
                    ...
                    {isCenter && (
                      <button
                        ...
                        className="absolute bottom-3.5 right-3.5 z-30 w-9 h-9 rounded-full bg-black/70 hover:bg-black/90 backdrop-blur-md border border-white/25 flex items-center justify-center text-white transition-all cursor-pointer shadow-xl hover:scale-110"
